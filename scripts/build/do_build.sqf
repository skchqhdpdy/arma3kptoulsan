	...
	if(buildtype == 1) then {
		...
		if ( buildtype == 8 ) then {
			...
			if (!(buildtype == 6 || buildtype == 99)) then {
				...
				{ [player, _classnamecfg] remoteExec ["remote_call_voteBuild",leader _x]; } forEach (_allgroups);
			...
			if ( build_confirmed == 2 ) then {
				...
				if(buildtype == 99) then {
					...
					format["%1 Build FOB %2", name player, [[] call F_getNearestFob] call F_getFobName ] remoteExec ["diag_log", 2];
				}
				else {
					...
					format["%1 Build %2 at FOB %3", name player, _classname, [_posfob] call F_getFobName ] remoteExec ["diag_log", 2];
				};
				if ( buildtype == 6 || buildtype == 99 ) then {
					_vehicle setVectorUp [0,0,1];
				} else {
