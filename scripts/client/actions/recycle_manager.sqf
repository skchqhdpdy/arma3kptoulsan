...
veh_action_distance = 25;

{
	_recycleable_classnames pushBack ( _x select 0 );
} foreach (light_vehicles + heavy_vehicles + air_vehicles + static_vehicles + support_vehicles + light_vehicles_extension + heavy_vehicles_extension + air_vehicles_extension + static_vehicles_extension + support_vehicles_extension );

_building_classnames = [];
{
	_building_classnames pushBack ( _x select 0 );
} foreach (buildings);
_building_classnames = _building_classnames 
+ [ 
