...
while { true } do {
	...
	_neararsenal = ( (getpos player) nearEntities [ [Arsenal_typename] , _distarsenal ]) select { getObjectType _x >= 8 };
	_nearfobbox = ( (getpos player) nearEntities [ [ FOB_box_typename, FOB_truck_typename ] , _distbuildfob ] );
	_nearspawn = ( (getpos player) nearEntities [ [huron_typename] , _distspawn ] );
