...
if ( isServer ) then {
	...
	if ( side _killer == GRLIB_side_friendly ) then {
		...
		if ( count _nearby_bigtown > 0 ) then {
			[0.5 * GRLIB_difficulty_modifier] call F_addCombatReadiness;
	...
	if ( _unit isKindOf "Man" ) then {
		if ( side (group _unit) == GRLIB_side_civilian ) then {
			...
			if ( isPlayer _killer ) then {
				...
				// Civilian Penalty 중복 집행으로 인한 각주화. check HS addscore
				/* if ( GRLIB_civ_penalties ) then {
					resources_ammo = resources_ammo - GRLIB_civ_killing_penalty;
					combat_readiness = combat_readiness + 10 + (floor (random 10));
		...
		if ( typeof _unit in all_hostile_classnames ) then {
			if ( combat_readiness > 0 ) then {
				[-floor (random 5)] call F_addCombatReadiness;
				};
