...
private _spawn_marker = [ 500, 999999, false ] call F_findOpforSpawnPoint;
...
private _pilotUnits = units _pilotsGrp;
{
	...
	[_x, true, true] call ace_captives_fnc_captureUnit;
...
if ( _alive_crew_count == 0 ) then {
	...
	{ [ _x ] spawn { sleep 30; deleteVehicle (_this select 0) } } foreach _pilotUnits;
};

resources_intel = resources_intel + (40 * _alive_crew_count);
